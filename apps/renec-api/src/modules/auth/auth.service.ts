...
export class AuthService {
  ...
  constructor(
    ...
    this.authCfg = new AuthConfig(this.config.get('guard'));
  }
  ...
  async verifyOtp(input: VerifyOtpDto): Promise<Otp> {
    ...
    if (!otp || otp.publicCode !== input.publicCode || otp.status !== OTPStatus.ACTIVATED) {
      throw new BadRequestException('SMS_CODE_INVALID_OR_EXPIRED');
    }
  ...
  async register(input: RegisterDto, clientInfo: ClientInfo): Promise<TokeResponseDto> {
    const existEmail = await this.userService.findByEmail(input.email);
    ...
    const gravatar: Gravatar = await getGravatar(input.email);
    const user = await this.userService.create({
      fullName: input.fullName || gravatar?.fullName,
      phone: input.phone,
      email: input.email,
